...
export function TrendApp({ onExit }: TrendAppProps) {
  ...
  if (!snapshot) {
    return (
      <Box padding={1}>
        <Text>Inicializando estratégia de tendência…</Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" paddingX={1} paddingY={0}>
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text>
          Exchange: {exchangeName} ｜ Par: {snapshot.symbol} ｜ Preço Recente: {formatNumber(lastPrice, 2)} ｜ SMA30: {formatNumber(sma30, 2)} ｜ Tendência: {trend}
        </Text>
