...
import { SmaRsiEngine, type SmaRsiEngineSnapshot } from "../strategy/sma-rsi-engine";
import { formatNumber } from "../utils/format";
import { DataTable, type TableColumn } from "./components/DataTable";

const READY_MESSAGE = "Aguardando dados da exchange…";

interface SmaRsiAppProps {
  onExit: () => void;
}

const inputSupported = Boolean(process.stdin && (process.stdin as any).isTTY);

export function SmaRsiApp({ onExit }: SmaRsiAppProps) {
  const [snapshot, setSnapshot] = useState<SmaRsiEngineSnapshot | null>(null);
  const [error, setError] = useState<Error | null>(null);
  const engineRef = useRef<SmaRsiEngine | null>(null);
  ...
  useEffect(() => {
    try {
      ...
      const engine = new SmaRsiEngine(tradingConfig, adapter);
      engineRef.current = engine;
      setSnapshot(engine.getSnapshot());
      const handler = (next: SmaRsiEngineSnapshot) => {
  ...
  if (!snapshot) {
    return (
      <Box padding={1}>
        <Text>Inicializando estratégia SMA30 + RSI…</Text>
      </Box>
    );
  }
